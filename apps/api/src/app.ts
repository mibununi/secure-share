...
app.get("/api/files/:fileId/permissions", requireAuth, async (req: AuthReq, res) => {
    try {
        ...
        const fileRes = await pool.query(
            `SELECT owner_id FROM files WHERE id = $1 LIMIT 1`,
            [fileId]
        );
...
app.post("/api/files/:fileId/share", requireAuth, async (req: AuthReq, res) => {
    try {
        ...
        const userCheck = await pool.query(`SELECT id FROM users WHERE id = $1 LIMIT 1`, [recipientUserId]);
...
app.post("/api/files/:fileId/revoke", requireAuth, async (req: AuthReq, res) => {
    try {
        ...
        const fileQ = await pool.query(`SELECT owner_id FROM files WHERE id = $1 LIMIT 1`, [fileId]);
        if (fileQ.rows.length === 0) return res.status(404).json({ ok: false, error: "File not found" });
        ...
        if (req.user!.role === "manager") {
            const target = await pool.query(`SELECT role FROM users WHERE id = $1 LIMIT 1`, [recipientUserId]);
